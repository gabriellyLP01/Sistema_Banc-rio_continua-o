...
def transferir():
    ...
    while True:
        try:
            conta = int(input("Digite o número da conta do destinatário: "))
    ...
    agora = datetime.datetime.now()
    extrato.append({"data_hora": agora.strftime("%d/%m/%Y %H:%M:%S") , "tipo": "Tranferência", "valor": valor_transferencia})
    print(f"Tranferência de R$ {valor_transferencia:.2f} realizado com sucesso para a conta {conta}.")
    print(f"Seu novo saldo é: R$ {saldo:.2f}")
